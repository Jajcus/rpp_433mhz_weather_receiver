...
async fn decode_bit<'a>(input: &'a DecoderInputRx<'a>) -> Result<bool, ()> {
    ...
    if pulse1.kind != PulseKind::High || pulse1.length < 500 || pulse1.length > 600 {
        return Err(());
    }

    let pulse2 = input.receive().await;

    if pulse2.kind != PulseKind::Low {
        return Err(());
    }

    if pulse2.length < 2000 {
        Err(())
    } else if pulse2.length < 2500 {
        Ok(false)
    } else if pulse2.length < 4000 {
        Err(())
    } else if pulse2.length < 4500 {
        Ok(true)
    } else {
        Err(())
    }
...
async fn decode_bits<'a, const S: usize, const N: usize>(
    input: &'a DecoderInputRx<'a>,
) -> Result<[u8; S], ()> {
    if S < N / 8 {
        panic!("S < N / 8")
    }
    ...
    'main: loop {
        ...
        loop {
            ...
            if got_packets == 3 {
                ...
                let temperature =
                    (((packet[2] as u32) << 4) | ((packet[3] as u32) >> 4)) as f32 / 10.0;
                let humidity =
                    ((((packet[3] & 0x0f) as u32) << 4) | ((packet[4] as u32) >> 4)) as f32;
                output
                    .send(DecodedData {
                        temperature,
                        humidity,
